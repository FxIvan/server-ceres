...
.post(async(req,res)=>{
    ...
    if( title !== undefined && comando !== unedfined && description !== undefined && distribution !== undefined){
        ...
        .then(()=>{
            log_file.write(`${timeMoment} | MSG | Peticion POST exitosa, se guardo el comando -> ${comando}\n`)
            res.json({message:'Guardado Exitosamente',status:201})
        })
        .catch(err=>{
            res.json({error:err,status:503})
            log_fileError.write(`${timeMoment} | ERR | ERROR en la peticion POST -> ${err} \n`)
        })
    }else{
        log_file.write(`${timeMoment} | MSG | Algunos de los Campos no fue ingresado \n`)
    }
